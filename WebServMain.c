...
unsigned WINAPI RequestHandler(void *arg)
{
	...
	if(strcmp(method, "GET"))		// 요청 방식 확인 : GET
		SendErrorMSG(hClntSock);
...
void SendData(SOCKET sock, char* ct, char* fileName)		// 데이터 전송 함수
{
...
void SendErrorMSG(SOCKET sock)		// 오류 발생 메시지 전달
{
	char protocol[] = "HTTP/1.0 400 Bad Request\r\n";
	char servName[] = "Server : My Web Server\r\n";
	char cntLen[] = "Content-length : 2048\r\n";
	char cntType[] = "Content-type : text/html\r\n\r\n";
	char content[] = "<html><head><title>2009182051 한승원의 네트워크</title></head>"
		"<body><font size=+5><br>오류 발생. 요청 파일명 및 요청 방식 재확인"
		"</font></body></html>";
